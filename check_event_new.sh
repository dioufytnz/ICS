...
curl --max-time 3.0 -s -H "Content-type: application/json" -H "Accept: vdn.dac.v1" --request GET  http://$MOXA_IP/api/slot/0/io/di/0/diStatus | cut -d"S" -f2 | cut -d":" -f2 | tr -d '\{\}' > /tmp/di0_temp

curl --max-time 3.0 -s -H "Content-type: application/json" -H "Accept: vdn.dac.v1" --request GET  http://$MOXA_IP/api/slot/0/io/di/1/diStatus | cut -d"S" -f2 | cut -d":" -f2 | tr -d '\{\}' > /tmp/di1_temp

curl --max-time 3.0 -s -H "Content-type: application/json" -H "Accept: vdn.dac.v1" --request GET  http://$MOXA_IP/api/slot/0/io/di/2/diStatus | cut -d"S" -f2 | cut -d":" -f2 | tr -d '\{\}' > /tmp/di2_temp

curl --max-time 3.0 -s -H "Content-type: application/json" -H "Accept: vdn.dac.v1" --request GET  http://$MOXA_IP/api/slot/0/io/di/3/diStatus | cut -d"S" -f2 | cut -d":" -f2 | tr -d '\{\}' > /tmp/di3_temp

curl --max-time 3.0 -s -H "Content-type: application/json" -H "Accept: vdn.dac.v1" --request GET  http://$MOXA_IP/api/slot/0/io/ai/0/aiValueRaw | cut -d"S" -f2 | cut -d":" -f2 | tr -d '\{\}' > /tmp/ai1

# curl --max-time 3.0 -s -H "Content-type: application/json" -H "Accept: vdn.dac.v1" --request GET  http://$MOXA_IP/api/slot/0/io/ai/0/aiValueRaw
...
if [ -f "/tmp/di0" ]; then
    ...
    if [ $di0_temp -eq $di0 ]; then
        ...
        json="{\"ID\":\"$name\",\"serial\":\"$serial\",\"DI1\":\"$di0_temp\",\"DI2\":\"$di1_temp\",\"DI3\":\"$di2_temp\",\"DI4\":\"$di3_temp\",\"AI1\":\"$ai1\"}"
        curl --max-time 6.0 -X POST -H "Content-Type: application/json" \
